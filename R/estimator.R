...
#' This calculates a smoothed coefficient semiparametric IV estimator.
#'
#' @param cbar
#' @param h
#' @param Y
...
#'
#' @examples
npiv_cbar_fn <- function(cbar,h,Y,X,C,M){
  omega <- sqrt((1/(2*h))*(abs(C - cbar) < h))
  omega[C == cbar] <- 0
  # if(cbar >= (1-h)){
  #    omega[ cbar >= (1-h)] <- omega / ((((1-cbar)/h)-1)*0.5)
  # }
  M_omega <- sweep(as.matrix(M),MARGIN = 1,omega, FUN = '*')
  X_omega <- sweep(as.matrix(X),MARGIN = 1,omega, FUN = '*')
  Y_omega <- sweep(as.matrix(Y),MARGIN = 1,omega, FUN = '*')
  # thetahat_r <- solve(crossprod(W_omega,W_omega),crossprod(W_omega,Y_omega))
  thetahat_r <- MASS::ginv(crossprod(M_omega,X_omega))%*%crossprod(M_omega,Y_omega)
  return(thetahat_r)
}


#' Compute average treatment effects of the model, where the localize step
#' computes a semiparametric smoothed coefficient IV estimator.
#'
#' @param h
#' @param Y
#' @param X
...
#'
#' @examples
npiv_fn <- function(h,Y,X,C,D,Z,C_g,rho_g,M){

  # Step 1: "Localize" Compute kernel functions for each city.
  thetahat_vec     <- sapply(C_g,function(cbar) npiv_cbar_fn(cbar,h,Y,X,C,M))
